...
pub struct LinkedListItem<T> {
	item: T,
	previous_item: Option<Rc<RefCell<LinkedListItem<T>>>>,
	next_item: Option<Rc<RefCell<LinkedListItem<T>>>>,
}
...
impl <'a, T> Iterator for LinkedListIterator<'a, T> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let current = self.linked_list.current.clone();
		if let Some(current) = current {
			if Rc::ptr_eq(&current, match self.direction {
			...
			match self.direction {
				Direction::Forward => self.linked_list.advance(),
				Direction::Backward => self.linked_list.advance_back(),
			}
			Some(current)
		} else {
			None
...
impl <'a, T> LinkedList<T> {
	pub fn iter(&'a mut self) -> LinkedListIterator<'a, T> {
		LinkedListIterator { direction: Direction::Forward, linked_list: self, finished: false }
	}

	pub fn iter_reverse(&'a mut self) -> LinkedListIterator<'a, T> {
		LinkedListIterator { direction: Direction::Backward, linked_list: self, finished: false }
	}
}